from sarif_issues import sarif_to_df, sarif_to_rule_map, IN_TOP_25, COLUMNS
from pathlib import Path
import json
import unittest
from sarif import loader # MS Sarif Tools Library

class TestSarifIssues(unittest.TestCase):

    def test_sarif_to_rule_map_bandit(self):
        bandit_rules = [
            {
                "id": "B324",
                "name": "hashlib",
    ...
    def test_sarif_to_rule_map_spotbugs(self):
        spotbugs_rules = [
            {
    ...
    def test_sarif_to_rule_map_fortify(self):
        fod_rules = [
            {
    ...
    def test_sarif_to_df_bandit_has_all_cols(self):
        bandit_sarif_path = Path("test_data\\bandit-results-jan27.sarif")
        df = sarif_to_df(bandit_sarif_path)
        self.assertEqual(len(df.columns), len(COLUMNS))
        for col in df.columns:
            self.assertTrue(col in COLUMNS)
    
    def test_sarif_to_df_fod_py_has_all_cols(self):
        fod_py_sarif_path = Path("test_data\\py-gh-fortify-sast-jan27.sarif")
        df = sarif_to_df(fod_py_sarif_path)
        self.assertEqual(len(df.columns), len(COLUMNS))
        for col in df.columns:
            self.assertTrue(col in COLUMNS)

    def test_sarif_to_df_fod_java_has_all_cols(self):
        fod_java_sarif_path = Path("test_data\\java-gh-fortify-sast-jan27.sarif")
        df = sarif_to_df(fod_java_sarif_path)
        self.assertEqual(len(df.columns), len(COLUMNS))
        for col in df.columns:
            self.assertTrue(col in COLUMNS)      
   
    def test_sarif_to_df_spotbugs_has_all_cols(self):
        spotbugs_sarif_path = Path("test_data\\spotbugsSarif-jan29.json")
        df = sarif_to_df(spotbugs_sarif_path) 
        self.assertEqual(len(df.columns), len(COLUMNS))
        for col in df.columns:
            self.assertTrue(col in COLUMNS)      

    def test_sarif_to_df_fod_py_has_all_results(self):
        fod_py_sarif_path = Path("test_data\\py-gh-fortify-sast-jan27.sarif")
        df = sarif_to_df(fod_py_sarif_path)
        expected_sarif = loader.load_sarif_file(fod_py_sarif_path)
        expected_results = expected_sarif.get_results()
        self.assertEqual(len(df), len(expected_results))
        
    def test_sarif_to_df_fod_java_has_all_results(self):
        fod_java_sarif_path = Path("test_data\\java-gh-fortify-sast-jan27.sarif")
        df = sarif_to_df(fod_java_sarif_path)
        expected_sarif = loader.load_sarif_file(fod_java_sarif_path)
        expected_results = expected_sarif.get_results()
        self.assertEqual(len(df), len(expected_results))
        
    def test_sarif_to_df_spotbugs(self):
        spotbugs_sarif_path = Path("test_data\\spotbugsSarif-jan29.json")
        df = sarif_to_df(spotbugs_sarif_path)
        expected_sarif = loader.load_sarif_file(spotbugs_sarif_path)
        expected_results = expected_sarif.get_results()
        self.assertEqual(len(df), len(expected_results))

    def test_get_metrics_fod_py(self):
        fod_py_sarif_path = Path("test_data\\py-gh-fortify-sast-jan27.sarif")
        ...
        num_critical = len(fod_py_df[ fod_py_df['severity'] == "CRITICAL" ])
        num_top_25 = len(fod_py_df[ fod_py_df[IN_TOP_25] == True ])
